...
void newmacro(const char *s, int len, int hasspecs) {
  if (nmacros == nalloced) {
    ...
    macros = (MACRO *)realloc(macros, nalloced * sizeof *macros);
    if (macros == NULL)
      bug("Out of memory");
  }
...
void parseCmdlineDefine(const char *s) {
  ...
  if (s[l] == '(') {
    ...
    do {
      ...
      if (i > l)
        argc++;
      macros[nmacros].argnames = (char **)realloc(macros[nmacros].argnames,
                                                  (argc + 1) * sizeof(char *));
      if (i > l) {
...
void sendout(const char *s, int l,
             int proc) /* only process the quotechar, that's all */
{
  int i;
...
void initthings(int argc, char **argv) {
  ...
  nmacros = 0;
  nalloced = 31;
  macros = (MACRO *)(nalloced * sizeof *macros);

  S = (SPECS *)malloc(sizeof *S);
  S->User = CUser;
  S->Meta = CMeta;
  S->comments = NULL;
  S->stack_next = NULL;
  ...
  C = (INPUTCONTEXT *)(sizeof *C);
  C->in = stdin;
  C->argc = 0;
  C->argv = NULL;
  C->filename = my_strdup("stdin");
  C->out = (OUTPUTCONTEXT *)malloc(sizeof *(C->out));
  C->out->f = stdout;
  C->out->bufsize = 0;
  C->lineno = 1;
...
#define DEPRECATED_WARNING                                                     \
  fprintf(stderr, "gpp: warning: deprecated option `%s'; use `-%s' instead\n", \
          *arg, *arg)
...
int findMetaArgs(int start, int *p1b, int *p1e, int *p2b, int *p2e, int *endm,
  ...
  *p1b = pos;
  ...
  if ((*argc) && SplicePossibleUser(&pos, p1e, &hyp_end1, &hyp_end2, argb, arge,
...
char *ProcessText(const char *buf, int l, int ambience) {
  ...
  s[l + 1] = 0;
  T = C;
  C = (INPUTCONTEXT *)malloc(sizeof *C);
  C->out = (OUTPUTCONTEXT *)malloc(sizeof *(C->out));
  C->in = NULL;
  C->argc = T->argc;
  C->argv = T->argv;
...
int DoArithmEval(char *buf, int pos1, int pos2, int *result) {
  ...
  if (SpliceInfix(buf, pos1, pos2, (char *)"||", &spl1, &spl2)) {
  ...
  if (SpliceInfix(buf, pos1, pos2, (char *)">=", &spl1, &spl2)) {
    if (!DoArithmEval(buf, pos1, spl1, &result1) ||
      ...
      *result =
          (result1 > 0) || ((result1 == 0) && (spl1 - pos1 >= pos2 - spl2));
    } else
      *result = (result1 >= result2);
    return 1;
  }
...
void ProcessModeCommand(int p1start, int p1end, int p2start, int p2end) {
  ...
  if (idequal(C->buf + p1start, p1end - p1start, "quote")) {
    ...
    if (!strcmp(opt, "1") || !my_strcasecmp(opt, "on"))
      S->stack_next->preservelf = 1;
    else if (!strcmp(opt, "0") || !my_strcasecmp(opt, "off"))
    ...
    if (nargs == 0)
      ...
      delete_comment(S->stack_next, my_strdup(args[0]));
 ...
 void DoInclude(char *file_name) {
  ...
  for (j = 0; (f == NULL) && (j < nincludedirs); j++) {
    incfile_name =
        (char *)realloc(incfile_name, len + strlen(includedir[j]) + 2);
...
int ParsePossibleMeta(void) {
  ...
  case 18: /* ERROR */
    replace_directive_with_blank_line(C->out->f);
    if (!commented[iflevel])
      bug(ProcessText(C->buf + p1start, (nparam == 2 ? p2end : p1end) - p1start,
                      FLAG_META));
    break;
  ...
  case 19: /* WARNING */
    ...
    if (!commented[iflevel]) {
      char *s;
      s = ProcessText(C->buf + p1start, (nparam == 2 ? p2end : p1end) - p1start,
                      FLAG_META);
      warning(s);
      free(s);
    }
  ...
  case 20: { /* DATE */
    ...
    fmt = ProcessText(C->buf + p1start, (nparam == 2 ? p2end : p1end) - p1start,
                      FLAG_META);
...
int ParsePossibleUser(void) {
  ...
  idstart = 1;
  id = 0;
  if (!SplicePossibleUser(&idstart, &idend, &sh_end, &lg_end, argb, arge, &argc,
                          1, &id, FLAG_USER))
    return -1;
...
/* copy SLASH-terminated name of the directory of fname */
 ...
 FILE *openInCurrentDir(const char *incfile) {
  ...
  absfile = (char *)calloc(strlen(C->filename) + strlen(incfile) + 1, 1);
...
void replace_directive_with_blank_line(FILE *f) {
  if ((include_directive_marker != NULL) && (f != NULL) && (!S->preservelf) &&
      (S->Meta.mArgE[0] == '\n')) {
    fprintf(f, "\n");
  }
}
